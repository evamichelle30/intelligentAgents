...
public class Strategy {
    ...
    public class EstimateSolutionStrategy {
         ...
         */
        public static Candidate addFirstTaskToSolution(Candidate solution) {
            // find vehicle with biggest capacity
            int biggestCapacity = 0;
            Vehicle biggestVehicle = null;
            for (List<Task> entry : solution.taskLists) {
                Vehicle vehicle = solution.vehicles.get(0); //add correct vehicle
                if (vehicle.capacity() > biggestCapacity) {
                    biggestVehicle = vehicle;
                    biggestCapacity = biggestVehicle.capacity();
                }
                // TODO: save this entry?
            }

            // add tasks and update task pair map
            ArrayList<PD_Action> taskModels = new ArrayList<>();
            // TODO: add task
            taskModels.add(new PD_Action(0, task));
            taskModels.add(new PD_Action(1, task));
            // TODO: have vehicle id map
            solution.taskLists.add(biggestVehicle.id(), taskModels);

            // calculate and update costs
            double distance = biggestVehicle.getCurrentCity().distanceTo(task.pickupCity) +
                    task.pickupCity.distanceTo(task.deliveryCity);
            double cost = distance * biggestVehicle.costPerKm();
            // TODO: add vehicle costs ? solution.getVehicleCostMap().put(biggestVehicle, cost);
            solution.updateCost(cost);
         ...
         */
        public static Candidate optimalSolutionWithTask(Candidate solution,
                                                            Task task) {
            double bestCost = Double.MAX_VALUE;
            Candidate bestSolution = null;
            ...
            for (Map.Entry<Vehicle, ArrayList<TaskModel>> entry : solution.getVehicleTasksMap().entrySet()) {
                ...
                for (int i = 0; i < tasks.size() + 1; i++) {   // i - position of a pick up task
                    for (int j = i + 1; j < tasks.size() + 2; j++) {   // j - position of a delivery task
                        ...
                        Candidate newSolution = new Candidate(solution);
         ...
         */
        private static double updateSolutionAndGetCost(Candidate solution, Vehicle vehicle) {
         ...
         * @return - solution with added task
         */
        public static Candidate addTaskToEnd(Candidate solution,
