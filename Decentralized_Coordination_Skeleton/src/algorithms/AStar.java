...
public class AStar {
	// We define a template.State comparator to find the state with the minimum cost
	...
	public static Plan aStarPlan(Vehicle vehicle, TaskSet available, State initialState) {
		...
		while (!Q.isEmpty()) {
			...
			// We add the node and its children if 1) this is the first time it is visited or 2) the cost has decreased
